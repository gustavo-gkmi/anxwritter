...
| `datetime_ac_forbids_visible` | `datetime` AttributeClass with `visible=True`. | ANB v9 does not render datetime values on the canvas, so the chrome would render with no value. Set `visible=False` and add an `extra_cfg.date_attribute_displays` entry referencing this AC to render the formatted date as a paired text sibling. See [attributes.md → Date attribute displays](attributes.md#date-attribute-displays). |
| `date_display_invalid` | Malformed `date_attribute_displays` entry. | `start` missing or undeclared, `start` AC isn't datetime or doesn't have `visible=False`; same for `end`; `end == start`; range mode (`end` set) without `name`; `format` not a valid `strftime`; `missing` not in `skip`/`substitute`/`truncate`/`error`; inner `attribute_class.name` or `.type` set; or `missing='error'` and an item has only one of two bounds. |
| `date_display_name_collision` | Synthesised sibling name collides. | The display's effective name (explicit `name` or `f"{start}{suffix}"`) collides with another explicit `AttributeClass`, or two displays resolve to the same sibling name. Set or change `name` to disambiguate. |
