...
- **Value enforcement** — declarative regex / closed-enum rules on entity ids, attribute values, and per-type required attributes; org-level policy that flags drift across data sources so the same identity stays matchable across charts
